...
if(wb.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
  ...
  wb=replaceOnce(wb,'async run(e,t,n,i,r,s,o,a,c,l,u){const h=l6d(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Gc.localMode});if(Gc.localMode){',
   'async run(e,t,n,i,r,s,o,a,c,l,u){const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);const h=l6d(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Gc.localMode||__chatgptLocal});if(Gc.localMode||__chatgptLocal){');
}
...
wb=addUsage(wb,{fn:'function eAy(e){const t=Bbp(119)',jsx:'KIy',
  original:'title:"Plan & Usage",children:[ln,Gt,dn,vn]',
  claudeOnly:'title:"Plan & Usage",children:[ln,Gt,dn,vn,KIy(__claudeUsageSection,{})]',
  symbols:{jsx:'KIy',useState:'Zur',useEffect:'iIy',card:'sb',zs:'Ts',bar:'kA',barStyle:'rdr'}});
wb=patchMaxMode(patchSubagentSettingsWorkbench(wb));
wb=patchSubagentBubbles(patchRemoteRouting(wb,'desktop','3.21.18'),'desktop','3.21.18');
wb=patchSubagentLifecycle(wb,'desktop','chatgpt-codex/','3.21.18');
...
if(glass.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
  ...
  glass=replaceOnce(glass,'async run(t,e,n,i,r,s,o,a,l,c,u){const d=Tum(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Rl.localMode});if(Rl.localMode){',
   'async run(t,e,n,i,r,s,o,a,l,c,u){const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);const d=Tum(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Rl.localMode||__chatgptLocal});if(Rl.localMode||__chatgptLocal){');
}
...
glass=addUsage(glass,{fn:'function Dik(t){const e=bBg(119)',jsx:'Rik',
  original:'title:"Plan & Usage",children:[pt,bt,gt,St]',
  claudeOnly:'title:"Plan & Usage",children:[pt,bt,gt,St,Rik(__claudeUsageSection,{})]',
  symbols:{jsx:'Rik',useState:'gds',useEffect:'Aik',card:'Pf',zs:'Ns',bar:'Fm',barStyle:'IEi'}});
glass=patchMaxMode(patchSubagentSettingsWorkbench(glass));
glass=patchSubagentBubbles(patchRemoteRouting(glass,'glass','3.21.18'),'glass','3.21.18');
glass=patchSubagentLifecycle(glass,'glass','chatgpt-codex/','3.21.18');
