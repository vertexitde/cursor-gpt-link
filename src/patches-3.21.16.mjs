...
if(wb.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
  ...
  wb=replaceOnce(wb,'async run(e,t,n,i,r,s,o,a,c,l,u){const h=c6d(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Gc.localMode});if(Gc.localMode){',
   'async run(e,t,n,i,r,s,o,a,c,l,u){const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);const h=c6d(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Gc.localMode||__chatgptLocal});if(Gc.localMode||__chatgptLocal){');
}
...
wb=addUsage(wb,{fn:'function JIy(e){const t=Fbp(119)',jsx:'qIy',
  original:'title:"Plan & Usage",children:[ln,Gt,dn,vn]',
  claudeOnly:'title:"Plan & Usage",children:[ln,Gt,dn,vn,qIy(__claudeUsageSection,{})]',
  symbols:{jsx:'qIy',useState:'Zur',useEffect:'nIy',card:'sb',zs:'Ts',bar:'kA',barStyle:'rdr'}});
wb=patchMaxMode(patchSubagentSettingsWorkbench(wb));
wb=patchSubagentBubbles(patchRemoteRouting(wb,'desktop','3.21.16'),'desktop','3.21.16');
wb=patchSubagentLifecycle(wb,'desktop','chatgpt-codex/','3.21.16');
...
if(glass.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
  ...
  glass=replaceOnce(glass,'async run(t,e,n,i,r,s,o,a,l,c,u){const d=Cum(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Rl.localMode});if(Rl.localMode){',
   'async run(t,e,n,i,r,s,o,a,l,c,u){const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);const d=Cum(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Rl.localMode||__chatgptLocal});if(Rl.localMode||__chatgptLocal){');
}
...
glass=addUsage(glass,{fn:'function Mik(t){const e=vBg(119)',jsx:'Aik',
  original:'title:"Plan & Usage",children:[pt,bt,gt,St]',
  claudeOnly:'title:"Plan & Usage",children:[pt,bt,gt,St,Aik(__claudeUsageSection,{})]',
  symbols:{jsx:'Aik',useState:'gds',useEffect:'Iik',card:'Pf',zs:'Ns',bar:'Fm',barStyle:'IEi'}});
glass=patchMaxMode(patchSubagentSettingsWorkbench(glass));
glass=patchSubagentBubbles(patchRemoteRouting(glass,'glass','3.21.16'),'glass','3.21.16');
glass=patchSubagentLifecycle(glass,'glass','chatgpt-codex/','3.21.16');
